...
const CW_EXTRA_TEMPLATE =
  ...
  '每一条 bullet 必须提供新的信息点，不能换个句式重复说同一件事——' +
  '这是当前所有 AI 生成 PPT 工具最常见的通病（内容看着齐全，读下来发现来回都在说同一句话，' +
  '信息密度低），写完之后自己检查一遍：如果拿掉某一条，前后条数少了会不会没影响，' +
  '如果没影响说明这一条是废话，要重写成真正有信息量的内容。' +
  ...
  '题目不要全是套公式直接算的类型，里面至少安排 1~2 道需要学生先判断/比较/说明理由' +
  '再动手算的启发式题目（比如"哪种算法更快，为什么"），公开课评比标准里"探究深度看' +
  '问题质量而不只是数量"说的就是这个。\n' +
