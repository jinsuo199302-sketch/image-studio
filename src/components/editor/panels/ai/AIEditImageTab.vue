...
import AIEditImageDialog from '../../AIEditImageDialog.vue'
...
function download() {
  ...
  saveFile('改图结果.png', workingImage.value)
}
</script>
  ...
  <div class="flex h-full flex-col">
    ...
    <div class="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">
      ...
      >
        ...
        <span class="text-xs">上传图片，用文字描述想怎么改</span>
      </div>
      ...
      <template v-else>
        ...
        <el-button class="!w-full" @click="editDialogOpen = true"> AI 改图（文字描述怎么改） </el-button>
      </template>
    </div>
    ...
    <AIEditImageDialog v-model="editDialogOpen" :image-src="workingImage ?? ''" @result="(url) => (workingImage = url)" />
  </div>
</template>
