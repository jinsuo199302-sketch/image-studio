...
_DECK_JSON_SPEC = (
    ...
    "content_image_prompt：正文页底图，浅色调、有科技感——装饰（细网格线、电路走线、斜切光带、半透明几何、淡光晕）主要放在**画面下方和左右两侧、四角**，"
    "顶部约 15% 和正中横向约 55% 高的区域要明显更淡更干净（顶部放标题、中间放文字卡片）。整体明亮、浅灰蓝白调、留白足，不要深色背景、不要照片、不要浓重色块。整篇正文页复用这一张。\n"
...
_DECK_ART_FALLBACK = {
    ...
    # 内容页底图：整篇复用同一张——要有科技感底纹，但顶部和中间放文字的区域必须淡
    "content": (
        "一张浅色调、有科技感的商业 PPT 正文页背景：整体是明亮的浅灰蓝白色调；"
        "科技装饰（细网格线 / 电路走线 / 斜切光带 / 半透明多边形 / 淡光晕）主要放在画面下方和左右两侧、四角；"
        "顶部约 15% 和正中间横向约 55% 高的区域要明显更淡、更干净（放标题和文字卡片，不能被压住）；"
        "整体克制、留白充足，不要浓重色块、不要深色背景、不要写实照片。"
    ),
}
