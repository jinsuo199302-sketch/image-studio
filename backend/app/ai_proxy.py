...
_DECK_JSON_SPEC = (
    ...
    '"speaker_notes":"这一页演讲时可以照着说的一段话,100~200字,口语化、像真人在讲不是在念稿,'
    '帮非专业讲述者也能流畅地把这页内容讲清楚;只能扩展/复述这页已有的 title/intro/bullets 信息,不能引入这页没有的新事实或数字"}]}]}\n'
...
async def _gen_deck_outline(
    ...
    if material:
        prompt = (
            ...
            "并给出配套视觉方案。\n"
            "【硬性要求·一字不删】这次的任务是「原文一字不减地搬上幻灯片，只做结构整理」，不是「提炼总结」：\n"
            "- 原文出现的每一句话、每一个信息点（症状、体征、数值、专有名词、诊断/术语措辞、举例、备注等）"
            "都必须逐字保留，不能删减、不能省略某一条、不能为了控制字数而概括/合并、不能换个说法改写、"
            "不能把意思不同的几句话揉成一句——比如原文分别写“大便如常”和“小便控制不佳”，"
            "就必须原样分两条列出，不能合并成“大小便都控制不佳”（那是说反了事实，不是精简）。\n"
            "- 你能做的只是「结构性整理」：分段落分配到哪一页、加小标题/加序号、调整语序把长句拆成几条 bullet——"
            "但拆出来的每条 bullet 拼起来必须等于原文，不能丢字、不能加字、不能替换专业术语的措辞。\n"
            "- 严禁新增资料里没有的事实、数据或观点，不要脑补。资料里出现的数字/占比要保留并可做成图表页。\n"
            f"章节数、每节 slide 数、每条 bullet 字数，下面 JSON 规范里给的都只是「正常情况下」的参考值——"
            f"这次以「一字不删地装下原文」为准，原文信息量大就多分章节、多分页、多拆 bullet，"
            f"不要为了凑数字上限而删减内容（资料结构清晰就按它自然的段落数来；不清晰再参考约 {sections} 个章节这个量级）。\n"
            f"{extra_line}{ref_line}"
            f"{_DECK_JSON_SPEC}{photo_rule}\n"
            f"【资料原文】\n{material}"
        )
    elif research_claims:
        # 结构/深度要求（比如课件模式的"按7个教学环节、每个环节要写多细"）优先级要跟"填课题"
        # 分支一致，所以 extra_line/ref_line 放在开头同样的位置——之前把"只能用已核实事实"这条
        # 硬性要求块放在 extra_line 前面，真实测试发现模型会把这条挤到后面的课件结构要求盖过去，
        # 生成结果既不按7个教学环节走、内容也比平时单薄。教训：新增的约束性 prompt 不能不假思索
        # 塞在最前面，要考虑跟已有的（尤其是来自 extra 的）结构性要求谁该拿到更高优先级。
        prompt = (
            ...
            "每条 20~45 字，具体、准确、书面语，不空话套话；title/heading 精炼——"
            "以上结构/深度/篇幅要求（包括上面如果有课件教学环节这类结构规定）照常执行，不因为下面这条约束而缩水。\n"
            "【硬性要求·具体事实只能用已核实的】下面【联网核实的知识点】是真实检索、抓取网页原文后"
            "逐字校验过的可信素材，每条都标了来源和原文引用——这条规则只管「具体事实」，不管「结构/篇幅」：\n"
            "- 涉及具体事实、数据、例题、结论这类内容时，必须基于这些素材，可以用自己的话解释、串联、"
            "扩写说明，但不能编造素材之外的具体事实——素材没提到的具体细节宁可不写，不能凭自己的知识"
            "编（哪怕你觉得那是对的）。\n"
            "- 不涉及具体新事实的内容（过渡句、总结句、教学环节该有的讲解组织、引导性提问、常识性的"
            "背景铺垫）不受此限制，可以正常展开，按上面的结构/篇幅要求写够深度和字数，不能用"
            "「素材不够」当借口把内容写得单薄或跳过要求的环节。\n"
            "- 素材条数有限，不需要每条都用上，也不需要每页都对应一条素材。\n"
            f"【联网核实的知识点】\n{research_claims}"
        )
    else:
        prompt = (
...
_DECK_ART_GUARD = (
    ...
    "不要整圈边框相框；不要低俗或敏感内容；"
    "不要画任何卡片/图标徽标/按钮/仪表盘/图表控件/进度条等具象 UI 元素——"
    "这些是前端代码按实际内容条数精确画出来叠在上面的，AI 自己画的卡片数量和位置跟正文对不上，"
    "会变成画面里多余的空壳；装饰只用抽象的几何色块、光效、线条、渐变、模糊纹理或写意剪影。"
)
_DECK_ART_FALLBACK = {
    "cover": (
