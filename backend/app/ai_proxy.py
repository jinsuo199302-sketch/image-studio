...
async def _gen_deck_outline(
    ...
    if material:
        prompt = (
            ...
            "【关键·不能丢事实】精简/合并同类项时，原文列举的每一条具体信息（症状、体征、数值、专有名词、"
            "诊断/术语措辞等）都必须逐条完整保留，不能为了控制字数而省略某一条，更不能把意思不同的几条"
            "揉成一句、导致语义变了或变得含糊——比如原文分别写“大便如常”和“小便控制不佳”，"
            "不能合并成“大小便都控制不佳”（这已经不是精简，是说反了事实）。"
            "拿不准某条能不能删/合并时，宁可多分一页、多列一条，也不要为了凑字数删内容。\n"
            f"章节数：资料结构清晰就按它自然的段落数（2~6 个）来；否则归纳成约 {sections} 个章节。"
            "每个 section 下 2~4 个 slides，普通 slide 3~5 条 bullets、每条 15~45 字——"
            "但如果原文某一部分信息量确实大（比如一长串并列症状/体征/检查结果），"
            "宁可多拆几条 bullet 或多分一页，也不要为了凑进字数上限而删减、合并出语义不同的句子。\n"
